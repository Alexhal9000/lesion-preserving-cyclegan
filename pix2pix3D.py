...
def downsample(filters, size, norm_type='batchnorm', apply_norm=True, apply_dropout=False, last_layer=False):
  ...
  if apply_dropout:
    ...
    #result.add(tf.keras.layers.GaussianNoise(0.1))
...
def unet_generator(output_channels, norm_type='batchnorm'):
  ...
  down_stack = [
      downsample(64, 4, norm_type),  # (bs, 192, 192, 64)
      downsample(128, 4, norm_type),  # (bs, 96, 96, 128)
      downsample(256, 4, norm_type),  # (bs, 48, 48, 256)
      downsample(256, 4, norm_type),  # (bs, 24, 24, 512)
      downsample(256, 4, norm_type, last_layer=True),  # (bs, 12, 12, 512)
  ]

  up_stack = [
      upsample(256, 4, norm_type, apply_dropout=True),  # (bs, 12, 12, 1024)
      upsample(256, 4, norm_type, apply_dropout=True),  # (bs, 24, 24, 1024)
      upsample(256, 4, norm_type, apply_dropout=True),  # (bs, 48, 48, 512)
  ...
  return tf.keras.Model(inputs=inputs, outputs=x), mid
...
def unet_generator_dual(output_channels, norm_type='batchnorm'):
  ...
  down_stack_a = [
      downsample(32, 4, norm_type, apply_norm=False, apply_dropout=True),  # (bs, 192, 192, 64)
      downsample(64, 4, norm_type, apply_dropout=True),  # (bs, 96, 96, 128)
      downsample(128, 4, norm_type, apply_dropout=True),  # (bs, 48, 48, 256)
      downsample(256, 4, norm_type),  # (bs, 24, 24, 512)
      downsample(256, 4, norm_type),  # (bs, 12, 12, 512)
      #downsample(512, 3, norm_type),  # (bs, 6, 6, 512)
  ]

  up_stack_a = [
      #upsample(512, 3, norm_type, apply_dropout=True),  # (bs, 6, 6, 1024)
      upsample(256, 4, norm_type),  # (bs, 12, 12, 1024)
      upsample(256, 4, norm_type),  # (bs, 24, 24, 1024)
      upsample(128, 4, norm_type),  # (bs, 48, 48, 512)
      upsample(64, 4, norm_type),  # (bs, 96, 96, 256)
      upsample(32, 4, norm_type),  # (bs, 192, 192, 128)
  ]

  initializer = tf.random_normal_initializer(0., 0.02)
  last_a = tf.keras.layers.Conv3DTranspose(
      output_channels, 4, strides=2,
      padding='same', kernel_initializer=initializer,
      activation='tanh')  # (bs, 256, 256, 3)
  ...
  inputs_a = tf.keras.layers.Input(shape=[None, None, None, 1])
  x = inputs_a

  # Downsampling through the model
  skips_a = []
  for down in down_stack_a:
    x = down(x)
    skips_a.append(x)

  skips_a = reversed(skips_a[:-1])

  mid_a = tf.keras.Model(inputs=inputs_a, outputs=x)

  # Upsampling and establishing the skip connections
  nn = 0
  for up, skip in zip(up_stack_a, skips_a):
    x = up(x)
    if nn >= 0:
      x = concat([x, skip])
    nn = nn + 1
  ...
  gen_a = tf.keras.Model(inputs=inputs_a, outputs=x)
  ...
  down_stack_b = [
      downsample(32, 4, norm_type, apply_norm=False, apply_dropout=True),  # (bs, 192, 192, 64)
      downsample(64, 4, norm_type, apply_dropout=True),  # (bs, 96, 96, 128)
      downsample(128, 4, norm_type),  # (bs, 48, 48, 256)
      downsample(256, 4, norm_type),  # (bs, 24, 24, 512)
      downsample(256, 4, norm_type),  # (bs, 12, 12, 512)
      #downsample(512, 3, norm_type),  # (bs, 6, 6, 512)
  ]
  ...
  initializer = tf.random_normal_initializer(0., 0.02)
  last_b = tf.keras.layers.Conv3DTranspose(
      output_channels, 4, strides=2,
      padding='same', kernel_initializer=initializer,
      activation='tanh')  # (bs, 256, 256, 3)
  ...
  inputs_b = tf.keras.layers.Input(shape=[None, None, None, 1])
  y = inputs_b

  # Downsampling through the model
  skips_b = []
  for down in down_stack_b:
    y = down(y)
    skips_b.append(y)

  skips_b = reversed(skips_b[:-1])

  mid_b = tf.keras.Model(inputs=inputs_b, outputs=y)

  # Upsampling and establishing the skip connections
  nn = 0
  for up, skip in zip(up_stack_b, skips_b):
    y = up(y)
    if nn >= 0:
      y = concat([y, skip])
    nn = nn + 1
  ...
  gen_b = tf.keras.Model(inputs=inputs_b, outputs=y)
  ...
  # Upsampling and establishing the skip connections
  nn = 0
  for up, skip in zip(up_stack_b, skips_a):
    xx = up(xx)
    if nn >= 0:
      xx = concat([xx, skip])
    nn = nn + 1
  ...
  auto_a = tf.keras.Model(inputs=inputs_a, outputs=xx)
  ...
  # Upsampling and establishing the skip connections
  nn = 0
  for up, skip in zip(up_stack_a, skips_b):
    yy = up(yy)
    if nn >= 0:
      yy = concat([yy, skip])
    nn = nn + 1
  ...
  auto_b = tf.keras.Model(inputs=inputs_b, outputs=yy)

  return gen_a, mid_a, auto_a, gen_b, mid_b, auto_b
...
def discriminator(norm_type='batchnorm', target=True):
  ...
  inp = tf.keras.layers.Input(shape=[None, None, None, 2], name='input_image')
  x = inp

  if target:
    tar = tf.keras.layers.Input(shape=[None, None, None, 2], name='target_image')
    x = tf.keras.layers.concatenate([inp, tar])  # (bs, 256, 256, channels*2)

  down0 = downsample(64, 4, norm_type, apply_norm=False)(x) 
  down1 = downsample(128, 4, norm_type)(down0) 
  down2 = downsample(256, 4, norm_type)(down1)
